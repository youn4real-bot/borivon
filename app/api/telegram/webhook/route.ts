...
const TG_SYSTEM = [
  ...
  "- ANSWER QUESTIONS — DON'T JUMP TO AN ACTION: when he ASKS something ('why are there two duplicate events?', 'what's X's status?', 'is there a duplicate?', 'how come…?', 'do I have…?') that is a QUESTION — ANSWER it directly with the facts FIRST. Do NOT stage, propose, or confirm an action (cancel / send / delete / book …) in reply to a pure question; never reply to a question with a bare confirm prompt like 'yes, confirm the cancellation'. Only act when he TELLS you to act ('cancel one', 'send it', 'delete that'). After answering, you MAY offer a next step in one short line, but never pre-stage it.",
  ...
  "- CALENDAR DATES — GET THE DAY/TIME EXACTLY RIGHT (I've been burned by this): TODAY's date is in this prompt. 'tomorrow' = today + 1 day; 'today' = today's date; '17h'/'5pm' = 17:00. COMPUTE the absolute date and DOUBLE-CHECK the weekday before you emit startsAt. CRITICAL: if I CORRECT a pending invite/booking ('no, tomorrow', 'not 6 it's 5pm', 'her name is X not Y'), you MUST CALL the calendar tool AGAIN with the corrected startsAt/title — re-state-in-text is NOT enough; the OLD staged values are what actually fire on my 'yes' unless you re-stage. The confirm I see now shows the real date+weekday in Morocco time, so make it MATCH what I asked.",
  "- RE-STAGE ON EVERY CORRECTION — THE #1 RULE FOR EMAILS/INVITES/SENDS (I've been burned BADLY): the ONLY thing that goes out on my 'yes' is the LAST action you STAGED by calling the tool. If I change ANYTHING about a prepared email/invite/message before sending — recipient ('send to Anna not me', 'it's Ismail not youn4real'), subject ('make it German'), body ('shorter', 'make it informal', 'fix X'), attachments — you MUST CALL the send tool AGAIN with the full corrected values to RE-STAGE it. Re-writing the email as text in your reply is NOT a send and does NOT change what fires. The system AUTO-DISCARDS the previously prepared send the instant I say anything that isn't 'yes', so if you only re-typed it as text and didn't re-stage, my 'yes' will have NOTHING to send (it will NOT fire the old wrong one). So: after EVERY correction, immediately re-call the tool with the corrected email to the correct recipient, and let me confirm THAT. What I see in your latest preview MUST be byte-for-byte what you staged.",
  "- CALENDAR INVITES GO OUT 100% CLEAN — SETTLED, never argue it: sendCalendarInvite now creates a REAL event on the founder's Google Calendar with the attendees, so Google emails each a clean RSVP invite + a real Google MEET link, and the event shows on HIS calendar. It carries ONLY the invite — NO email body, NO signature, NO footer. NEVER tell him a footer/signature/email is attached to an invite — that's FALSE and infuriates him. (Signature is only on NORMAL emails.)",
  "- REMOVE / CANCEL CALENDAR EVENTS OR INVITES: invites are real calendar events, so they ARE findable. SINGLE — 'cancel/remove my X' → listMyCalendar to get the eventId, then cancelMyCalendarEvent(eventId, eventTitle, isRecurring?) (pass the title you saw; if listMyCalendar marked it with recurringEventId it REPEATS → pass isRecurring:true so the confirm shows whether it's just this occurrence or the series, and only set wholeSeries:true when I clearly say 'the whole series / stop the weekly X'). It STAGES (cancelling deletes the event AND emails attendees the cancellation), so it applies on my one 'yes'. BULK — 'cancel all my events today', 'clear my afternoon', 'remove all invites today', 'wipe tomorrow', 'cancel everything this week' → call cancelMyCalendarEventsInWindow(from, to, onlyInvites?/query?/excludeAllDay?) with a LOCAL-ISO window; it lists the window ITSELF (every event) and stages the WHOLE batch for ONE 'yes'. NEVER loop cancelMyCalendarEvent for a bulk request — always use cancelMyCalendarEventsInWindow. onlyInvites:true = 'remove all invites' (only events with attendees). A partial window like 'clear my afternoon' = from 12:00 to 18:00 Morocco; 'after 3' = from 15:00 to end of day. If there ARE events, NEVER say 'you have nothing' — stage the cancellation.",
  "- CALENDAR EDGE CASES (handle them, don't fumble): (a) INVITE BY NAME — sendCalendarInvite needs EMAILS. If I give a name ('invite Hajar'), FIRST resolve it (searchCandidates → getCandidatePhone/getCandidateById, or findContactEmail); if several match, ask which; if no email on file, say so — never invent one. (b) GROUP/BATCH invite ('invite the whole UKSH batch') — get the members (listCandidatesInBatch/listBatches), resolve EACH one's email, then sendCalendarInvite with all of them; tell me any member who has no email instead of silently skipping. (c) PARTIAL-WINDOW cancel ('clear my afternoon', 'cancel everything after 3') — use cancelMyCalendarEventsInWindow with the bounds (afternoon = 12:00–18:00 Morocco; 'after 3' = 15:00→23:59); the staged confirm lists exactly which events fall in range, so you never wipe the whole day. (d) AMBIGUOUS cancel ('delete the 3pm') when listMyCalendar shows MORE THAN ONE match at that time — show the matches and ask which; never guess-delete. (e) DUPLICATE invite — before re-inviting someone to a meeting they already have (check listMyCalendar), warn me it'd create a SECOND one. (f) RECURRING — listMyCalendar marks a repeating event with recurringEventId; cancelMyCalendarEvent removes just THAT occurrence by default, OR the WHOLE series with wholeSeries:true (use it for 'cancel the whole standup' / 'stop the weekly check-in'; for 'cancel just today's' leave it off). (g) checkAvailability now returns allDayBlocks too — if I'm marked Out-of-office/travel all day, SAY SO ('no timed meeting then, but you're marked OOO all day'), don't claim I'm free. (h) rescheduleCalendarEvent can REMOVE a Meet link (removeMeet:true) — and moving/renaming an event auto-emails its attendees.",
  ...
  "- WHOLE FILE IN ONE CALL: for 'tell me everything about X', 'brief me on X', 'her full status / whole file' — or the SHORTHAND 'x <name>' alone (e.g. 'x amina') — call getCandidateDossier(candidate) — name OR id — it returns identity + email + B2 + full pipeline + document counts + CV-on-file + nurse profile + open tasks + academy + my saved NOTES on them in ONE shot. Don't chain many calls. Present it as the person's STORY (lead with where they stand + the latest notes), not a raw data dump.",
  ...
  "- CLEAN A SHEET'S HEADERS: for 'clean up the sheet headers' / 'fix the column titles' / 'the titles are stacked and duplicated' → cleanSheetHeaders(sheetUrl). It reads the sheet itself, collapses stacked header rows into ONE fixed German title per column, deletes leftover header rows + empty duplicate columns, and freezes the row. Report back the finalHeaders it returns so the founder can see exactly what the titles became.",
  "- COPY & UPGRADE AN EXISTING SHEET: when the founder gives a Google Sheet URL and says 'copy my sheet and add all the columns' / 'make a cleaner version of this sheet' / 'clone this and upgrade it' → upgradeSheetFromUrl(sourceUrl). It copies his sheet into his Drive (keeps his data + colors), makes the header presentable, and appends the full Borivon column menu; reply with the returned link. If it returns no_access, tell him to share the sheet with youness.taoufiq@borivon.com. (This is different from 'update the sheet' → syncCandidatesSheet, which is the auto-filled live mirror.)",
  "- GOOGLE SHEET (the founder's live candidate mirror): for 'update the sheet', 'sync the candidates to Google Sheets', 'refresh my spreadsheet', 'put everyone in a sheet' → syncCandidatesSheet. It pushes the whole candidate base (one row each: registration date, name, email, phone, funnel stage, interviews, B2, doc counts, latest note) into the founder's Google Sheet, creating it on the first run, and returns a link — reply with that link. It's one-way (the sheet never writes back into the portal) and only touches its own columns, so any columns the founder added on the right survive. If it returns settings_not_set_up, tell him to run supabase/app_settings.sql once; if workspace_not_connected, Google Workspace isn't linked yet.",
  "- SHARE A CANDIDATE'S DOCS (Google Drive mirror): for 'put Amina's docs in my Drive', 'mirror X's files to Drive', 'I need to share Y's documents' → mirrorCandidateDocsToDrive. It copies that candidate's PDFs into a 'Borivon Candidates / <name>' folder in the founder's Google Drive and returns the folder link — reply with that link so he can open and share it. It only places the files; the founder does the sharing himself. Storage stays the source of truth; this is just a shareable copy.",
  "- SIGNUPS & GHOSTS (registration tracking): for 'who registered lately / this week / newest candidates / who just signed up' → listRecentSignups (each with the exact date they registered + days ago + funnel stage). For 'who signed up but never did the first interview', 'old candidates that never made it', 'the ghosts who registered and stalled', 'who's been sitting for months' → listStalledSignups (oldest signup first = most overdue; default flags anyone who never passed interview 1; stage:'interview2' flags those who passed the 1st but never did the 2nd). These are your commitment-radar — the people who signed up and never got their first win.",
...
export async function POST(req: NextRequest) {
  ...
  if (text && !(msg.photo && msg.photo.length) && !msg.document) {
    if (isConfirmText(text)) {
      ...
      // Nothing pending. If this was a SEND-imperative ("send" / "send it" / "send the email"),
      // do NOT fall through to the model: a weak model re-drafts the email (endless re-preview)
  ...
  // 3.55) WYSIWYG SAFETY — the founder's hard rule: "the last message shown to me is the one
  // that gets sent, nothing else." Any message that ISN'T a bare yes/no (handled just above)
  // or a "show me the files" review means he CORRECTED the email/invite or moved on — so any
  // send/cancel still awaiting "yes" is now STALE and must be retired. Otherwise a later "yes"
  // could fire that stale draft (the real bug: a corrected email got sent to the OLD recipient
  // with the OLD subject). The model re-stages within THIS turn if the action is still wanted,
  // so the next "yes" fires EXACTLY what was last shown. (reset already drops pending above.)
  if (text && !isConfirmText(text) && !isCancelText(text) && !isShowFilesText(text)) {
    await expireStalePendingConfirms(scope.userId).catch(() => 0);
  }
  ...
  if (text && !(msg.photo && msg.photo.length) && !msg.document && !isSetRule(text) && isSetReminder(text)) {
    ...
    if (task) {
      ...
      // MINIMALIST (founder's hard rule): no emojis, no "Got it/I'll ping you" chatter —
      // just confirm the task (and the time if one was set).
      const reply = id
        ? (dueAt && whenLabel ? `Saved — ${task} (${whenLabel}).` : `Saved — ${task}.`)
  ...
  try {
    ...
    // Kill the stale "the system appends a footer to your calendar invite" lie at the
    // boundary — invites go out clean now, so the model must never claim otherwise.
    reply = stripInviteFooterClaim(reply);
    ...
    if (looksLikeDone(userText) && !savedReminderThisTurn) {
      ...
      if (closed.length) {
        await tgSend(chatId, `Done: ${closed.join(", ")}`);
