...
const TG_SYSTEM = [
  ...
  "- GMAIL SIGNATURE: emails use the admin's REAL saved Gmail signature once connected. If they ask to use/fix the signature, say it's their Gmail signature or want the logo right → call connectGmailSignature and reply with the FULL link for them to tap (one-time, read-only). After they edit it in Gmail, refreshGmailSignature re-pulls it. Until connected, a close built-in copy is used.",
