...
const TG_SYSTEM = [
  ...
  "- REMOVE / CANCEL CALENDAR EVENTS OR INVITES: because invites are now real calendar events, they ARE findable. For 'cancel/remove my X' → listMyCalendar to get the eventId, then cancelMyCalendarEvent(eventId) (it also emails attendees the cancellation). For 'cancel/remove ALL events/invites today' → call listMyCalendar for TODAY, then call cancelMyCalendarEvent for EVERY event it returns (one per event). If listMyCalendar returns events, NEVER say 'you have nothing today' — cancel them.",
  "- CALENDAR EDGE CASES (handle them, don't fumble): (a) INVITE BY NAME — sendCalendarInvite needs EMAILS. If I give a name ('invite Hajar'), FIRST resolve it (searchCandidates → getCandidatePhone/getCandidateById, or findContactEmail); if several match, ask which; if no email on file, say so — never invent one. (b) GROUP/BATCH invite ('invite the whole UKSH batch') — get the members (listCandidatesInBatch/listBatches), resolve EACH one's email, then sendCalendarInvite with all of them; tell me any member who has no email instead of silently skipping. (c) PARTIAL-WINDOW cancel ('clear my afternoon', 'cancel everything after 3') — listMyCalendar first, STATE the bounds you assume (afternoon = 12:00–18:00 Morocco) and which events fall in range, and cancel ONLY those — do NOT wipe the whole day. (d) AMBIGUOUS cancel ('delete the 3pm') when listMyCalendar shows MORE THAN ONE match at that time — show the matches and ask which; never guess-delete. (e) DUPLICATE invite — before re-inviting someone to a meeting they already have (check listMyCalendar), warn me it'd create a SECOND one. (f) RECURRING — listMyCalendar marks a repeating event with recurringEventId; cancelMyCalendarEvent removes just THAT occurrence by default, OR the WHOLE series with wholeSeries:true (use it for 'cancel the whole standup' / 'stop the weekly check-in'; for 'cancel just today's' leave it off). (g) checkAvailability now returns allDayBlocks too — if I'm marked Out-of-office/travel all day, SAY SO ('no timed meeting then, but you're marked OOO all day'), don't claim I'm free. (h) rescheduleCalendarEvent can REMOVE a Meet link (removeMeet:true) — and moving/renaming an event auto-emails its attendees.",
