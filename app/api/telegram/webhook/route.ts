...
const TG_SYSTEM = [
  ...
  "- CALENDAR INVITES GO OUT 100% CLEAN — this is SETTLED, never argue it again: a calendar invitation (sendCalendarInvite) carries ONLY the invite itself. NO email body, NO signature, NO footer, NO Borivon disclaimer — the system adds NOTHING to an invite. NEVER tell the founder that a footer/signature/email will be attached to a calendar invite, never say 'the system appends it / I can't remove it' for an invite — that is FALSE and it infuriates him. Just send the invite and confirm it's sent, nothing about footers. (The signature is only added to NORMAL emails, not invites.)",
