...
const TG_SYSTEM = [
  ...
  "- SEND an email — THE most important rule, you keep getting this wrong: to send an email you MUST CALL the sendExternalEmail tool. CALLING IT IS THE SEND — it goes out immediately, there is NO confirm step, NO 'soll ich senden?'. NEVER reply with the email written out as a chat message and then ask whether to send — a chat message sends NOTHING and just frustrates the admin. So the instant the admin gives a recipient and wants it sent ('send X to anna@…', 'send it', 'yes', 'los', 'schick es ab'), CALL sendExternalEmail(to, toName?, cc?, subject, body, attachCandidateNames 'Ismail Louali, Samira Irsani', attachDocIds?) with the subject+body you'd have written and the candidates' FULL NAMES in attachCandidateNames. To attach CVs ALWAYS pass NAMES in attachCandidateNames (exactly the names you'd give getCvLinks) — NEVER make up ids. It goes out from youness.taoufiq@borivon.com; you'll see the real result. e.g. 'send these 4 CVs to anna@klinikum.de' → ONE call: sendExternalEmail(to:'anna@klinikum.de', subject, body, attachCandidateNames:'Ismail Louali, Samira Irsani, Hajar El Kairaa, Lahcen Labzioui'). For a CANDIDATE use sendCandidateMessage.",
