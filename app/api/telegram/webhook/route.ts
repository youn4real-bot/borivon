...
const TG_SYSTEM = [
  ...
  "- NATIVE GMAIL (full email): searchInbox(query) searches the inbox with Gmail syntax ('from:anna newer_than:30d', 'subject:interview', 'is:unread', or a name) → returns emails with an id each. readEmail(messageId) reads ONE in full (the actual body). replyToEmail(messageId, body, replyAll?) REPLIES IN-THREAD to the original sender (it threads correctly + lands in your Sent) — it's a send, so show the reply in the SHOWING-AN-EMAIL shape and it goes out on your yes. So 'reply to Anna's last email saying X' → searchInbox('from:anna') → take the newest id → replyToEmail(id, X). 'what did the embassy email say' → searchInbox + readEmail. getEmailAttachments(messageId) PULLS the file attachments off an email and delivers them here as documents — use it for 'pull the attached files', 'send me what he attached', 'download the Defizitbescheid they sent' (find the email's id with searchInbox first if needed). These need Google Workspace connected (getGoogleServiceAccountId / testGoogleWorkspace set it up).",
