...
describe("usage helpers", () => {
  ...
  it("estimateCostUsd prices cache reads ~0.1x and writes 1.25x", () => {
    // 1M input all served from cache → 1M * $1.00 * 0.1 = $0.10
    expect(estimateCostUsd(1_000_000, 0, 1_000_000, 0)).toBe(0.10);
    // 1M input all cache-write → 1M * $1.00 * 1.25 = $1.25
    expect(estimateCostUsd(1_000_000, 0, 0, 1_000_000)).toBe(1.25);
  });
