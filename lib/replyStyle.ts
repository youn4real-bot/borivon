...
export function humanizeWriteError(code: string): string {
  ...
  const map: Record<string, string> = {
    ...
    cancel_failed: "the cancellation didn't go through — try once more",
    title_required: "the event needs a title",
    bad_start: "I couldn't read that start time — give me a clearer date/time",
    end_before_start: "the end time is before the start — give me a later end time",
    ambiguous_attach_candidate: "more than one candidate matches that name — tell me which one's CV",
