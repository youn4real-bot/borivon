...
export type MapRow = { userId: string; name: string; photo: string | null; status: Status; sellable?: { sellable: boolean }; b2Stage?: string; b2Failed?: boolean; anerkennungStage?: string; impfungStage?: string; impfungDoses?: { got: number; need: number }; needsUpdate?: boolean; stuck?: { stuck: boolean } };
...
function Dot({ r, ringColor, halo, index = 0, badge, dragRef, dragHandle, isDragging, draggable }: {
  ...
  return (
    ...
    >
      ...
      {badge}
      {/* Chase signal on the avatar: a pulsing RED ⏳ when the candidate is STUCK
          too long at their current station (per-stage budget) — the stronger
          call to chase. Otherwise the beloved weekly ⚡ when there's simply been
          no update in a week. Mutually exclusive so the dot never gets busy. */}
      {r.stuck?.stuck ? (
        <motion.span aria-hidden
          style={{ position: "absolute", top: -5, right: -5, fontSize: 13, lineHeight: 1, pointerEvents: "none", filter: "drop-shadow(0 0 2px rgba(239,68,68,0.95))" }}
          animate={reduce ? undefined : { scale: [1, 1.3, 1], opacity: [0.85, 1, 0.85] }}
          transition={{ duration: 1.1, repeat: Infinity, ease: "easeInOut" }}>⏳</motion.span>
      ) : r.needsUpdate ? (
        <motion.span aria-hidden
